...
- Re-verified and expanded `surfaces.md`, `claude-code.md`, `agent-skills.md`, `source-policy.md` against current docs; corrected the stale claim that claude.ai Skills never reach Claude Code.
- Evals: `questions.json` schema v2 (`should_trigger`, `reads`), 12 new cases (7 should-not-trigger), and a generated `claude plugin eval` suite with trigger, routing and rubric graders; `scripts/summarize_evals.py` reports recall, false-trigger rate and routing accuracy.
